...
func TestOTelRuntimeConfigFromEnv(t *testing.T) {
	lookup := func(key string) (string, bool) {
		values := map[string]string{
			"SQUAD_OTEL_ENABLED":         "true",
			"SQUAD_OTEL_ENDPOINT":        "127.0.0.1:4317",
			"SQUAD_OTEL_INSECURE":        "true",
			"SQUAD_OTEL_HEADERS":         "authorization=Bearer token,x-tenant=demo",
			"SQUAD_OTEL_BATCH_TIMEOUT":   "250ms",
			"SQUAD_OTEL_SERVICE_VERSION": "1.2.3",
		}
		value, ok := values[key]
		return value, ok
	}

	cfg, enabled, err := observability.OTelRuntimeConfigFromEnv(lookup, observability.OTelRuntimeConfig{
		ServiceName: "agent-squad-demo",
		TracerName:  "agent-squad-demo",
	})
	if err != nil {
		t.Fatalf("expected env config to parse: %v", err)
	}
	if !enabled {
		t.Fatal("expected env config to enable OpenTelemetry")
	}
	if cfg.Endpoint != "127.0.0.1:4317" {
		t.Fatalf("expected endpoint to be loaded, got %q", cfg.Endpoint)
	}
	if !cfg.Insecure {
		t.Fatal("expected insecure mode to be true")
	}
	if cfg.BatchTimeout != 250*time.Millisecond {
		t.Fatalf("expected batch timeout 250ms, got %s", cfg.BatchTimeout)
	}
...
func TestOTelRuntimeConfigFromEnvDisabled(t *testing.T) {
	lookup := func(key string) (string, bool) {
		if key == "SQUAD_OTEL_ENABLED" {
			return "false", true
		}
		return "", false
	}

	_, enabled, err := observability.OTelRuntimeConfigFromEnv(lookup, observability.OTelRuntimeConfig{ServiceName: "agent-squad-demo"})
	if err != nil {
		t.Fatalf("expected disabled config to be accepted: %v", err)
	}
	if enabled {
		t.Fatal("expected OpenTelemetry to remain disabled")
	}
}
